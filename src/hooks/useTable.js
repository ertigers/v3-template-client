...
export const useTable = (api, initParam = {}, isPageable = true, dataCallBack = null, requestError) => {
  ...
  const pageParam = computed({
    get: () => {
      return {
        pageNumber: state.value.pageable.pageNumber,
  ...
  const getTableList = async () => {
    ...
    try {
      ...
      state.value.tableData = isPageable ? res.data.content : res.data;
      // 解构后台返回的分页数据 (如果有分页更新分页信息)
      if (isPageable) state.value.pageable.total = res.data.totalElements;
      // 简单判断data是否树结构
      if (res.data?.content?.find(item => item?.children || item?.child || item?._children)) isTreeData.value = true;
    } catch (error) {
      console.log({ error });
   ...
   * */
  const search = () => {
    state.value.pageable.pageNumber = 1;
    updatedTotalParam();
    getTableList();
  };
  ...
  const handleCurrentChange = val => {
    state.value.pageable.pageNumber = val;
    getTableList();
  };
