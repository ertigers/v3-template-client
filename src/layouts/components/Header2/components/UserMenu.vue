<template>
  <div>
    <el-popover placement="right" :width="160" popper-class="user-popover">
      <template #reference>
        <div class="toolBar-icon user-trigger">
          ...
          />
          <span v-if="!collapse" class="user-text">{{ username }}</span>
          <el-icon v-if="!collapse" class="arrow-icon"><ArrowRight /></el-icon>
        </div>
      </template>

      <div class="user-menu">
        <div class="user-menu-item" @click="openDialog('infoRef')">
          <svg-icon
            class="menu-icon"
            name="MenuUser"
            :icon-style="{
