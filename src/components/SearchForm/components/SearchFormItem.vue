...
const placeholder = computed(() => {
  ...
  if (["datetimerange", "daterange", "monthrange"].includes(search?.props?.type) || search?.props?.isRange) {
    return {
      rangeSeparator: search?.props?.rangeSeparator ?? $t("global.to"),
      startPlaceholder: search?.props?.startPlaceholder ?? $t("global.startTime"),
      endPlaceholder: search?.props?.endPlaceholder ?? $t("global.endTime")
    };
  }
  const placeholder =
    search?.props?.placeholder ??
    (search?.el?.includes("input") ? $t("global.placeholderInput") : $t("global.placeholderSelect"));
  return { placeholder };
});
