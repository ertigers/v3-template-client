  ...
  <div class="upload-box">
    ...
    >
      <template v-if="imageUrl">
        <img :src="getImageUrl(imageUrl, storeName)" class="upload-image" />
        <div class="upload-handle" @click.stop>
          ...
          <div class="handle-icon" @click="imgViewVisible = true">
            ...
            <span>{{ $t("components.Upload.preview") }}</span>
          </div>
    ...
    <el-image-viewer
      v-if="imgViewVisible"
      :url-list="[getImageUrl(imageUrl, storeName)]"
...
import { uploadImg, getImageUrl } from "@/api/modules/upload";
...
const handleHttpUpload = async options => {
  ...
  formData.append("store", props.storeName);
  try {
    const api = props.api ?? uploadImg;
    const { data } = await api(formData);
    emit("update:imageUrl", data.file_id);
...
const beforeUpload = rawFile => {
  ...
  if (!imgType)
    ElNotification({
      ...
      message: t("components.Upload.invalidType"),
      type: "warning"
    });
  if (!imgSize)
    setTimeout(() => {
      ElNotification({
        title: t("global.notifyTitle"),
        message: t("components.Upload.limitTip") + props.fileSize + "M",
        type: "warning"
      });
    }, 0);
...
const uploadError = () => {
  ElNotification({
    title: t("global.notifyTitle"),
    message: t("components.Upload.uploadError"),
    type: "error"
  });
};
</script>
